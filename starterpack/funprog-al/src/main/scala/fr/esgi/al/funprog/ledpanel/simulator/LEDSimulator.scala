...
object LEDSimulator {
  ...
  def simulate(panel: Panel, instructions: List[Instruction])
    ...
    // on verif les conflit avant l'etat final
    checkAllConflicts(sortedInstructions) match {
      case Some(error) => Left[ExecutionError, SimulationState](error)
      case None        => executeInstructions(initialState, sortedInstructions)
    }
  }

  private def checkAllConflicts(
      instructions: List[Instruction]): Option[ExecutionError] = {
    val instructionsByTime = instructions.groupBy(_.time)

    instructionsByTime.toList.flatMap { case (time, instructionsAtTime) =>
      val allPositions = instructionsAtTime.flatMap(_.getPositions)
      val positionCounts =
        allPositions.groupBy(identity).view.mapValues(_.length)

      positionCounts.toList.collectFirst {
        case ((x, y), count) if count > 1 =>
          MultipleInstructionsAtSameTime(time, x, y)
      }
    }.headOption
  }
  ...
  private def updateCumulativeTime(
      state: SimulationState,
      newTime: Int): SimulationState = {
  ...
  private def applyInstructionToPositions(
    ...
    val initialValue: Either[ExecutionError, SimulationState] =
      Right[ExecutionError, SimulationState](state)
  ...
  private def applyInstructionToPosition(
    ...
    if (!state.panel.isValidPosition(x, y)) {
      Left[ExecutionError, SimulationState](
        PositionOutOfBounds(x, y, state.panel.width, state.panel.height)
      )
    } else {
        ...
        case None =>
          ...
          LED.create(x, y, instruction.color, 0.0) match {
              ...
              applyAction(newLED, instruction.action, instruction.color) match {
                case Right(updatedLED) =>
                  state.panel.updateLED(updatedLED) match {
                    case Right(updatedPanel) =>
                      Right[ExecutionError, SimulationState](
                        state.copy(panel = updatedPanel)
                      )
                    case Left(_) =>
                      Left[ExecutionError, SimulationState](
                        PositionOutOfBounds(
            ...
            case Left(_) =>
              ...
              Left[ExecutionError, SimulationState](
                InvalidIntensityChange(x, y, 0.0, actionName)
              )
          }
  ...
  private def applyAction(
    ...
    action match {
      ...
      case Action.Decrement =>
        if (led.intensity <= 0.0) {
          ...
          led.withIntensity(0.0) match {
            ...
            case Left(_) =>
              Left[ExecutionError, LED](
                InvalidIntensityChange(led.x, led.y, led.intensity, "decrement")
              )
          }
        }
      ...
      case Action.Switch =>
        ...
        val colorResult: Either[ExecutionError, LED] =
          if (newIntensity > 0.0 && led.isOff) {
            led.withColor(newColor) match {
              case Right(coloredLED) => Right[ExecutionError, LED](coloredLED)
              case Left(_)           => Right[ExecutionError, LED](led)
            }
        ...
        colorResult match {
          case Right(coloredLED) =>
            coloredLED.withIntensity(newIntensity) match {
              ...
              case Left(_) =>
                Left[ExecutionError, LED](
                  InvalidIntensityChange(led.x, led.y, led.intensity, "switch")
                )
            }
          case Left(execError) => Left[ExecutionError, LED](execError)
        }
