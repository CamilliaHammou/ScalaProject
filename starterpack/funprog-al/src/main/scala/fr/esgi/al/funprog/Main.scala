...
def readFile(path: String): Either[SimulationError, List[String]] = {
  ...
  tryResult match {
    ...
    case Failure(_) =>
      Left[SimulationError, List[String]](
        FileReadError(path, "Error reading file")
      )
  }
}
def parseFile(lines: List[String])
    : Either[SimulationError, ((Int, Int), List[Instruction])] = {
  lines match {
    case Nil =>
      Left[SimulationError, ((Int, Int), List[Instruction])](
        InvalidDimensions("Empty file")
      )
...
def parseInstructions(
    lines: List[String]): Either[SimulationError, List[Instruction]] = {
  val nonEmptyLines = lines.filter(line => line.trim.nonEmpty)

  def processLine(line: String, acc: Either[SimulationError, List[Instruction]])
      : Either[SimulationError, List[Instruction]] = {
    acc match {
      case Right(accList) =>
        InstructionParser.parseInstruction(line) match {
          case Right(instruction) =>
            Right[SimulationError, List[Instruction]](instruction :: accList)
  ...
  val initialAcc =
    Right[SimulationError, List[Instruction]](List.empty[Instruction])
...
def createPanel(width: Int, height: Int): Either[SimulationError, Panel] = {
  Panel.create(width, height) match {
    ...
    case Left(err) =>
      Left[SimulationError, Panel](InvalidDimensions(s"$width x $height: $err"))
  }
}
def runSimulation(panel: Panel, instructions: List[Instruction])
...
@main
def MainWithArgs(args: String*): Unit = {
  args.toList match {
    case "--mode=tiling" :: n :: m :: Nil =>
      fr.esgi.al.funprog.tiling.TilingCalculator
        .displayTilingPossibilities(n.toInt, m.toInt)
    ...
    case _ =>
      println(
        "Usage : sbt runMain fr.esgi.al.funprog.MainWithArgs <fichier>  ou  sbt runMain fr.esgi.al.funprog.MainWithArgs --mode=tiling <n> <m>"
      )
  }
}
