...
final case class InvalidInstruction(line: String, reason: String)
    extends ParseError {
...
final case class InvalidColor(colorStr: String) extends ParseError {
  def message: String =
    s"Invalid color: '$colorStr'. Must be r,g,b with values 0 or 1"
}
...
final case class PositionOutOfBounds(x: Int, y: Int, width: Int, height: Int)
    extends ExecutionError {
  def message: String =
    s"Position ($x, $y) is out of bounds for panel ${width}x${height}"
}
final case class MultipleInstructionsAtSameTime(time: Int, x: Int, y: Int)
    extends ExecutionError {
...
final case class InvalidIntensityChange(
    x: Int,
    y: Int,
    ...
    extends ExecutionError {
  def message: String =
    s"Invalid intensity change for LED ($x, $y): current=$currentIntensity, action=$action"
}
final case class SimultaneousColorAndIntensityChange(x: Int, y: Int)
    extends ExecutionError {
  def message: String =
    s"Cannot change color and intensity simultaneously for LED ($x, $y)"
}
