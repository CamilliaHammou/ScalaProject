...
//erreur parsing du fichier d'entrée
sealed trait ParseError extends SimulationError
case class InvalidDimensions(input: String) extends ParseError {
  def message: String = s"Invalid panel dimensions: '$input'"
}
case class InvalidInstruction(line: String, reason: String) extends ParseError {
  def message: String = s"Invalid instruction '$line': $reason"
}
case class InvalidTime(timeStr: String) extends ParseError {
  def message: String = s"Invalid time value: '$timeStr'"
}
case class InvalidAction(actionStr: String) extends ParseError {
  def message: String = s"Invalid action: '$actionStr'. Must be +, -, or %"
}
case class InvalidColor(colorStr: String) extends ParseError {
  def message: String = s"Invalid color: '$colorStr'. Must be r,g,b with values 0 or 1"
}
case class InvalidPosition(posStr: String) extends ParseError {
  def message: String = s"Invalid position: '$posStr'"
}

//erreur de simulation
sealed trait ExecutionError extends SimulationError
case class PositionOutOfBounds(x: Int, y: Int, width: Int, height: Int) extends ExecutionError {
  def message: String = s"Position ($x, $y) is out of bounds for panel ${width}x${height}"
}
case class MultipleInstructionsAtSameTime(time: Int, x: Int, y: Int) extends ExecutionError {
  def message: String = s"Multiple instructions for LED ($x, $y) at time $time"
}
case class ColorChangeOnActiveLED(x: Int, y: Int) extends ExecutionError {
  def message: String = s"Cannot change color of active LED at ($x, $y)"
}
case class InvalidIntensityChange(x: Int, y: Int, currentIntensity: Double, action: String) extends ExecutionError {
  def message: String = s"Invalid intensity change for LED ($x, $y): current=$currentIntensity, action=$action"
}
case class SimultaneousColorAndIntensityChange(x: Int, y: Int) extends ExecutionError {
...
case class FileReadError(path: String, reason: String) extends FileError {
